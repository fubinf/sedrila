...
def test_student_work_so_far():
    def preparations():
        ...
        commit("%A 1h", "%B 1h"),
        request_grading("A", "B")
        grade({"A": c.SUBMISSION_REJECT_MARK})  # should not count double
        grade({"A": c.SUBMISSION_REJECT_MARK, "B": c.SUBMISSION_REJECT_MARK})
        commit("%A 1h", "%B 1h", "%Task1 1h", "%Task2 1h")
        request_grading("A", "B")
        grade({"A": c.SUBMISSION_ACCEPT_MARK, "B": c.SUBMISSION_CHECK_MARK}, signed=False)  # student can't cheat
        # grading unrequested tasks should not be allowed
        grade({"A": c.SUBMISSION_REJECT_MARK, "B": c.SUBMISSION_REJECT_MARK, 
               "Task1": c.SUBMISSION_REJECT_MARK, "Task2": c.SUBMISSION_ACCEPT_MARK})
        commit("%A 1h", "%B 1h")
        request_grading("A", "B")
        grade({"A": c.SUBMISSION_REJECT_MARK, "B": c.SUBMISSION_ACCEPT_MARK})
        request_grading("A")
...
def test_submitting_requirement():
    def preparations():
        ...
        # we have rejected B completely, students should now be allowed to do requiresB
        grade({"requiresB": c.SUBMISSION_ACCEPT_MARK})
...
def test_grading_mistakes():
    def preparations(override):
        ...
        request_grading("A", "B")
        grade({"A": (c.SUBMISSION_OVERRIDE_PREFIX if override else "") + c.SUBMISSION_ACCEPT_MARK, 
    ...
    run_inside_repo(lambda: preparations(False), lambda course: assertions(course, False), 
                    coursemodifications)
    run_inside_repo(lambda: preparations(True), lambda course: assertions(course, True), 
                    coursemodifications)
