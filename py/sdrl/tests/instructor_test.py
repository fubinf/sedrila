...
def test_instructor_parts(capfd):
    ...
    with tb.TempDirEnvironContextMgr(**{c.REPO_USER_CMD_VAR: "echo SEDRILA_INSTRUCTOR_COMMAND was called"}) as mgr:
        ...
        assert entries[0] == r.ReportEntry('Task1', 'basis/linuxcli/Task1', 1.0, 1.0, 0, False)
        # ----- rewrite submission file:
        # the repo's submission file contains dict(Task1="CHECK", NonExistingTask3="CHECK")
        b._testmode_reset()
        sut.rewrite_submission_file(course, c.SUBMISSION_FILE)
        submission = b.slurp_yaml(c.SUBMISSION_FILE)
        assert submission['NonExistingTask3'] == c.SUBMISSION_NONTASK_MARK
        ...
        if False:  # due to the kludge with USER_CMD_VAR above , this part is broken 
            assert f"{c.REPO_USER_CMD_VAR} was called" in output  # check some of the explanation
            assert os.environ[c.REPO_USER_CMD_VAR] in output  # make sure the actual command is shown
        is_valid = sut.validate_submission_file(course, c.SUBMISSION_FILE)
        assert not is_valid
        output = capfd.readouterr().out
        ...
        submission['Task1'] = c.SUBMISSION_ACCEPT_MARK  # accept the other entry
        b.spit_yaml(c.SUBMISSION_FILE, submission)
        with unittest.mock.patch('time.sleep'):
            sut.call_instructor_cmd(course, sut.instructor_cmd(), iteration=1)
        output = capfd.readouterr().out
        if False:  # due to the kludge with USER_CMD_VAR above , this part is broken 
            assert f"Calling '{os.environ[c.REPO_USER_CMD_VAR]}' again" in output  # the repeat blurb
        assert sut.validate_submission_file(course, c.SUBMISSION_FILE)
        # ----- finish:
        
        def pseudo_push():
            assert last_commit.hash != git.commits_of_local_repo()[0].hash
            git.discard_commits(1)  # undo the c.SUBMISSION_FILE commit
        ...
        with unittest.mock.patch('git.push', new=pseudo_push):
            sut.commit_and_push(c.SUBMISSION_FILE)
        output = capfd.readouterr().out
